...
procedure BajaFisica (var a_empleados: archivo);
var
	codigoBaja, direccionBaja: integer;
	actual: tipoEmpleados;
begin
	write ('Ingrese el DNI del empleado a dar de baja: ');
	readln (codigoBaja);
	reset (a_empleados);

	read (a_empleados, actual);

	while ((not EOF(a_empleados)) and (actual.DNI <> codigoBaja)) do 
			read (a_empleados, actual);
	writeln ('check 1');
	if (actual.DNI = codigoBaja) then begin
		direccionBaja:= FilePos(a_empleados) - 1;
		writeln ('check 2');
		seek (a_empleados, FIleSize(a_empleados)-1);
		writeln ('check 3');
		read (a_empleados, actual);
		writeln ('check 4');
		seek (a_empleados, direccionBaja);
		write (a_empleados, actual);
		seek (a_empleados, FileSize(a_empleados) - 1);
		truncate (a_empleados);
	
	
	end else writeln ('Dicho DNI no se encuentra en el archivo');
	close (a_empleados);
end;
...
while aux <> 9 do begin
	...
	writeln ('          8-  Eliminar un empleado (baja fisica)');
	writeln ('          9-  Volver.');
	read (aux);
	case aux of
