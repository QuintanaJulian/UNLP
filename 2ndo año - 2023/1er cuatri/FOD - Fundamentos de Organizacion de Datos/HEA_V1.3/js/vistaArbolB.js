...
function rotularNodo(divPadre, posicionContenedor) {
  ...
  if (idNodoPadre == "N00") {
    ...
    var coordenadaXNodo =
      parseFloat(posicionNodo.left) - parseFloat(posicionContenedor.left) - 50;
    var coordenadaYNodo =
      parseFloat(posicionNodo.top) - parseFloat(posicionContenedor.top) + 15;
    contexto.fillText(
      " Nodo " + divPadre.style.zIndex,
      coordenadaXNodo,
      coordenadaYNodo
    );
  }
...
function dibujarEnlaceEntreNodos(divPadre, posicionContenedor) {
  if (divPadre.childNodes.length - 1 <= ordenGlobal) {
    ...
    for (var x = 1; x <= divPadre.childNodes.length - 1; x++) {
      ...
      var coordenadaXPuntero =
        parseFloat(posicionPuntero.left) - parseFloat(posicionContenedor.left);
      var coordenadaYPuntero =
        parseFloat(posicionPuntero.top) -
        parseFloat(posicionContenedor.top) +
        30;
      contexto.moveTo(coordenadaXPuntero, coordenadaYPuntero);
      var coordenadaXHijo =
        parseFloat(posicionHijo.left) -
        parseFloat(posicionContenedor.left) +
        parseFloat(anchoTotalNodo / 2);
      var coordenadaYHijo =
        parseFloat(posicionHijo.top) - parseFloat(posicionContenedor.top);
  ...
  $("#contenedor")
    ...
    .each(function (i) {
      ...
      if (primerPos == "N") {
        ...
        if (divPadre.childNodes.length > 1) {
          //el nodo tiene hijos
...
function crearArbol(orden, maxElementos) {
  ...
  anchoTotalNodo =
    anchoElemento * maxElementos + anchoPuntero * (maxElementos + 1);
...
function partirRaiz(elementoAPromocionar) {
  ...
  if (nodoRaiz.childNodes.length > 1) {
    ...
    setTimeout("centrarHijosEnFuncionAPadre('N00')", 200);
    setTimeout(
      "finalizarDistribucion('E00','E10','E11',elementoAPromocionar)",
      velocidad_transicion
    );
    alert(valocidad_transicion);
  } else {
    /*creo el nodo izquierdo*/
    var numeroNodo = "N10";
    var nuevoTop = alturaEntreNodos;
  ...
  $("#" + idNodo)
    ...
    .each(function (i) {
      ...
      if (primerPos == "E") {
        ...
        var valor =
          "E" +
          alturaElementoDecrementada +
          idActual.substr(2, idActual.length - 2);
        $(this).attr("id", valor);
      } else {
        if (primerPos == "P") {
          ...
          var valor =
            "P" +
            alturaPunteroDecrementada +
            idActual.substr(2, idActual.length - 3) +
            idActual.substr(idActual.length - 1);
          $(this).attr("id", valor);
        } else {
          if (primerPos == "N") {
            ...
            var valor =
              alturaContenidoDecrementada +
              idActual.substr(1, idActual.length - 2) +
              idActual.substr(idActual.length - 1);
            $(this).attr("id", valor);
          }
        }
  ...
  for (var x = 0; x <= nodoOverFlow.childNodes.length - 1; x++) {
    if (divParcial.hasChildNodes()) {
      if (divParcial.firstChild.nodeValue < elementoAPromocionar) {
        ...
        divParcialIzquierdo.appendChild(
          document.createTextNode(divParcial.firstChild.nodeValue)
        );
...
function finalizarDistribucion(
  ...
  nodoOverFlow.firstChild.nextSibling.appendChild(
    document.createTextNode(elementoAPromocionar)
  );
  ...
  var divNuevoNodoIzquierdo = document.getElementById(
    "explicacionNuevoNodoIzquierdo"
  );
  nodoIzquierdo.removeChild(divNuevoNodoIzquierdo);
  var divNuevoNodoDerecho = document.getElementById(
    "explicacionNuevoNodoDerecho"
  );
...
function partirNodo(
  ...
  numeroNodoDesbordado = parseInt(
    numeroNodoDesbordado.substr(2, numeroNodoDesbordado.length - 2)
  );
  ...
  divEnglobaDesbordadoProvisorio = document.getElementById(
    idNodoDesbordado + "Provisorio"
  );
  /*creo los dos divs que me representan la division*/
  /*creo el nodo izquierdo*/
  numeroNodo = idNodoDesbordado;
  ancho =
    (maxElementos / 2) * anchoElemento +
    (maxElementos / 2) * anchoPuntero +
    anchoPuntero;
  nuevoLeftIzquierdo = -ancho;
  nuevoTop = 35;
  crearNuevoNodoDivision(
    numeroNodo,
    nuevoTop,
    nuevoLeftIzquierdo,
    "izquierdo",
    idNodoDesbordado + "Provisorio",
    idDivEnglobaDesbordado + "Provisorio",
    idDivEnglobaDesbordado
  );
  /*creo el nodo derecho*/
  proximoNodo = nodo + 1;
  ...
  crearNuevoNodoDivision(
    ...
    idNodoDesbordado + "Provisorio",
    idDivEnglobaDesbordado + "Provisorio",
    idEnglobaDesbordadoDerecho
  );
  /*distribuyo los elementos del nodo que desbordo a los nuevos nodos crados por la division*/
  idDivEnglobaDesbordadoParametro =
    "E" +
    idEnglobaDesbordadoDerecho[1] +
    idEnglobaDesbordadoDerecho.substr(2, idEnglobaDesbordadoDerecho.length - 2);
  distribuirElementos(
    idNodoDesbordado + "Provisorio",
    idNodoDesbordado,
    idDivEnglobaDesbordadoParametro,
  ...
  if (divPadreNodo.lastChild.previousSibling.firstChild == undefined) {
    //significa que el nodo a donde promociona no desborda
    idDivPadreNodo = divPadreNodo.id;
    pos = idDivPadreNodo.substr(2, idDivPadreNodo.length - 2);
    setTimeout("ordenarNodo(alturaNodoPadre,pos,elementoAPromocionar)", 200);
  }
  ...
  alturaNodo = altura;
  setTimeout(
    "finalizarPartirNodo(idDivEnglobaDesbordado+'Provisorio',idDivEnglobaDesbordado,idEnglobaDesbordadoDerecho,idPadreNodoDesbordado,numeroNodo,alturaNodo)",
    velocidad_transicion
  );
  ...
  var mensaje =
    "- Desborde del nodo " +
    numeroNodoMensaje +
    ". Divisi\u00f3n y promoci\u00f3n del elemento " +
    elementoAPromocionar;
  mostrarMensajeOperacion(mensaje);
}

/**FUNCION QUE CREA UN NUEVO NODO POR EL EFECTO DE DESBORDE*/
function crearNuevoNodoDivision(
  numeroNodo,
  nuevoTop,
  nuevoLeft,
  posicionNodo,
  idNodoPosterior,
  idDivEnglobaProvisorio,
  idDivEngloba
) {
  /*me quedo con el nodo que engloba al nodo desbordado*/
  var divEnglobaDesbordadoProvisorio = document.getElementById(
    idDivEnglobaProvisorio
  );
  /*me quedo con el div padre*/
  ...
  if (posicionNodo == "izquierdo") {
    divEnglobaDesbordadoNuevo.style.zIndex =
      divEnglobaDesbordadoProvisorio.style.zIndex;
  } else {
    if (vectorNumeracion.length == 0) {
  ...
  var divElementos = document.createElement("div");
  divElementos.id =
    "E" + idDivEngloba[1] + idDivEngloba.substr(2, idDivEngloba.length - 2);
  /*creo los divs que representan los punteros y los elementos*/
  for (var x = 0; x <= maxElementos - 1; x++) {
    var divPuntero = document.createElement("div");
    divPuntero.id =
      "P" +
      idDivEngloba[1] +
      idDivEngloba.substr(2, idDivEngloba.length - 2) +
      x;
  ...
  divPuntero = document.createElement("div");
  divPuntero.id =
    "P" + idDivEngloba[1] + idDivEngloba.substr(2, idDivEngloba.length - 2) + x;
  divPuntero.style.width = 10;
  divPuntero.style.height = 30;
  divPuntero.className = "puntero";
  divPuntero.style.position = "relative";
...
function acomodarRamas() {
  ...
  if (raiz.firstChild.nextSibling.childNodes.length > 1) {
    if (raiz.childNodes.length == 3) {
      //la raiz tiene solo dos hijos
      ...
      var limiteDerecho =
        parseFloat(raiz.style.left) + anchoTotalNodo + distanciaEntreRamas;
      var anchoRamaIzquierda = calcularAnchoADerechoRama(
        raiz.firstChild.nextSibling
      );
      ...
      if (anchoRamaIzquierda > limiteIzquierdo) {
        ...
        $("#" + idRamaIzquierda).animate(
          { left: "-=" + movimientoIzquierdo + "px" },
          1000
        );
        //acomodo el arbol
      ...
      if (anchoRamaDerecha < limiteDerecho) {
        ...
        $("#" + idRamaDerecha).animate(
          { left: "+=" + movimientoDerecho + "px" },
          1000
        );
      } else {
      ...
      for (var x = 1; x <= raiz.childNodes.length - 2; x++) {
        ...
        var limiteIzquierdo =
          parseFloat(anchoRamaDerecha) - distanciaEntreRamas;
        if (anchoRamaIzquierda > limiteIzquierdo) {
          var movimiento =
            anchoRamaIzquierda - limiteIzquierdo + distanciaEntreRamas;
...
function moverArbolParaScroll() {
  ...
  var posicionConDezplasamiento =
    parseInt(posicionReal.left) + parseInt(contenedor.scrollLeft);
  if (parseInt(posicionConDezplasamiento) < limiteIzquierdoPantalla) {
    var movimientoArbolCompleto =
      limiteIzquierdoPantalla -
      parseInt(posicionConDezplasamiento) +
      anchoElemento;
    $("#N00").animate(
      { left: "+=" + movimientoArbolCompleto + "px" },
      1250,
      function () {
        dibujarEnlaces();
      }
...
function centrarHijosEnFuncionAPadre(idNodoPadre) {
  ...
  var ancho =
    cantidadHijos * anchoTotalNodo + (cantidadHijos - 1) * distanciaEntreNodos;
  ...
  //acomodo los hijos
  posicionHijoMasIzquierdo =
    posicionHijoMasIzquierdo + distancia + anchoTotalNodo / 2;
...
function corrimientoADerecha(idDivNuevoNodoDerecho) {
  ...
  numeroNodoLimite = parseInt(
    numeroNodoLimite.substr(2, numeroNodoLimite.length - 2)
  );
  /*me quedo con el div padre*/
  ...
  if (divACorrer.childNodes.length == 1) {
    for (var x = 0; x <= divPadre.childNodes.length - 2; x++) {
      ...
      numeroNodoACorrer = parseInt(
        numeroNodoACorrer.substr(2, numeroNodoACorrer.length - 2)
      );
      if (parseInt(numeroNodoACorrer) > parseInt(numeroNodoLimite)) {
        var movimiento =
          parseInt(divACorrer.previousSibling.style.left) +
          anchoTotalNodo +
          distanciaEntreNodos;
    ...
    for (var x = 0; x <= divPadre.childNodes.length - 2; x++) {
      ...
      var numeroNodoACorrer = parseInt(
        idDivACorrer.substr(2, idDivACorrer.length - 2)
      );
      if (parseInt(numeroNodoACorrer) > parseInt(numeroNodoLimite)) {
        var movimiento =
          calcularMovimientoEnFuncionAHijosIzquierdos(idDivACorrer);
        divACorrer.style.left = movimiento;
      }
      divACorrer = divACorrer.nextSibling;
    }
...
function calcularMovimientoEnFuncionAHijosIzquierdos(numeroACorrer) {
  ...
  if (distanciaEntreExtremos < 0) {
    var movimiento =
      parseFloat(divACorrer.style.left) +
      (leftDivExtremoDerecho - leftDivExtremoIzquierdo) +
      distanciaEntreNodos;
  } else {
    if (distanciaEntreExtremos > distanciaEntreNodos) {
      ...
      var movimiento =
        parseFloat(divACorrer.style.left) +
        (distanciaEntreNodos - distanciaEntreExtremos);
    }
  }
  return movimiento;
}
  ...
  if (divACorrer.childNodes.length == 1) {
    for (var x = 0; x <= divPadre.childNodes.length - 2; x++) {
      ...
      if (parseInt(numeroNodoACorrer) < parseInt(numeroNodoLimite)) {
        ...
        movimiento =
          parseInt(divACorrer.nextSibling.style.left) -
          anchoTotalNodo -
          distanciaEntreNodos;
        divACorrer.style.left = movimiento;
      }
...
function calcularMovimientoEnFuncionAHijosDerechos(numeroACorrer) {
  ...
  if (distanciaEntreExtremos < 0) {
    var movimiento =
      parseFloat(divACorrer.style.left) -
      (leftDivExtremoDerecho - leftDivExtremoIzquierdo + distanciaEntreNodos);
  } else {
    if (distanciaEntreExtremos > distanciaEntreNodos) {
      var movimiento = distanciaEntreExtremos - distanciaEntreNodos;
      movimiento = parseFloat(divACorrer.style.left) + movimiento;
...
function distribuirHijos(
  ...
  for (var x = 1; x <= cantHijos; x++) {
    elemento =
      divParcial.firstChild.firstChild.nextSibling.firstChild.nodeValue;
    if (elemento < elementoAPromocionar) {
      //el hijo va al nodo izquierdo
        ...
        for (var h = 0; h <= divElementos.childNodes.length - 1; h++) {
          ...
          if (idElemento[0] == "P") {
            divElemento.id =
              ...
              altura +
              parseInt(j + 1) +
              idElemento[idElemento.length - 1];
          } else {
            divElemento.id =
              "" + altura + parseInt(j + 1) + idElemento[idElemento.length - 1];
          }
...
function ordenarNodoEliminacion(
  altura,
  nodo,
  posicionElemento,
  elementoAEliminar
  ...
  for (var x = 1; x <= maxElementos; x++) {
    if (divElemento.hasChildNodes()) {
      if (
        parseInt(divElemento.firstChild.nodeValue) > parseInt(elementoAEliminar)
      ) {
...
function borrarElementoDeNodoIntermedioConUnderADer(
  altura,
  nodo,
  posicionElemento,
  elementoIntermedioPadre
) {
  var idElementoAEliminar = "" + altura + nodo + posicionElemento;
  var divElementoAEliminar = document.getElementById(idElementoAEliminar);
  var elementoAEliminar = divElementoAEliminar.firstChild.nodeValue;
  ...
  var idContenedorElementosHermanoIzquierdo =
    contenedorElementosHermanoIzquierdo.id;
  //me quedo con el div del elemento del padre que pasa al nodo en underflow
  var idDivElementoPadre = buscarDivElementoDivisor(
    contenedorElementosPadre,
    elementoIntermedioPadre
  );
  var alturaDivElementoPadre = idDivElementoPadre[0];
  var nodoDivElementoPadre = idDivElementoPadre.substr(
    1,
    idDivElementoPadre.length - 2
  );
  var posicionDivElementoPadre = idDivElementoPadre.substr(
    idDivElementoPadre.length - 1
  );
  //me quedo con el div del elemento del hermano Izquierdo que pasa al padre
  var divElementoHermanoIzquierdo = obtenerUltimoElementoDeNodo(
    contenedorElementosHermanoIzquierdo
  );
  var elementoHermanoIzquierdo =
    divElementoHermanoIzquierdo.firstChild.nodeValue;
  ...
  var nodoDivElementoHermanoIzquierdo = idElementoHermanoIzquierdo.substr(
    1,
    idElementoHermanoIzquierdo.length - 2
  );
  var posicionDivElementoHermanoIzquierdo = idElementoHermanoIzquierdo.substr(
    idElementoHermanoIzquierdo.length - 1
  );
  ...
  $("#" + idContenedorElementosHermanoIzquierdo + "").effect(
    "pulsate",
    { times: 5 },
    700,
    ejecutarProximaAnimacion
...
function borrarElementoDeNodoIntermedioConUnderAIzq(
  altura,
  nodo,
  posicionElemento,
  elementoIntermedioPadre
  ...
  var idContenedorElementosHermanoDerecho =
    contenedorElementosHermanoDerecho.id;
  //me quedo con el div del elemento del padre que pasa al nodo en underflow
  var idDivElementoPadre = buscarDivElementoDivisor(
    contenedorElementosPadre,
    elementoIntermedioPadre
  ...
  var divElementoHermanoDerecho =
    contenedorElementosHermanoDerecho.firstChild.nextSibling;
  ...
  var nodoDivElementoHeramnoDerecho = idElementoHermanoDerecho.substr(
    1,
    idElementoHermanoDerecho.length - 2
  );
  var posicionDivElementoHermanoDerecho = idElementoHermanoDerecho.substr(
    idElementoHermanoDerecho.length - 1
  );
  ...
  $("#" + idContenedorElementos + "").effect("pulsate", { times: 5 }, 700);
  $("#" + idContenedorElementosHermanoDerecho + "").effect(
    "pulsate",
    { times: 5 },
    700,
    ejecutarProximaAnimacion
...
function borrarElementoDeNodoHojaConUnderAIzq(
  altura,
  nodo,
  posicionElemento,
  elementoIntermedioPadre
  ...
  var idElementoHermanoDerecho = divElementoHermanoDerecho.id;
  var alturaDivElementoHermanoDerecho = idElementoHermanoDerecho[0];
  var nodoDivElementoHermanoDerecho = idElementoHermanoDerecho.substr(
    1,
    idElementoHermanoDerecho.length - 2
  );
  var posicionDivElementoHermanoDerecho = idElementoHermanoDerecho.substr(
...
function redistribuirAIzq(
  ...
  borrarElementoDeNodoHojaSinUnder(
    alturaDivElementoHermanoDerecho,
    nodoDivElementoHeramnoDerecho,
    posicionDivElementoHermanoDerecho
  );
  //borro el elemento del padre que pasa al nodo en underflow
  borrarElementoDeNodoHojaSinUnder(
    alturaDivElementoPadre,
    nodoDivElementoPadre,
    posicionDivElementoPadre
  );
  //inserto el elemento del hermano derecho en el padre
  ordenarNodo(
    alturaDivElementoPadre,
    nodoDivElementoPadre,
    elementoHermanoDerecho
  );
...
function redistribuirADer(
  ...
  borrarElementoDeNodoHojaSinUnder(
    alturaDivElementoHermanoIzquierdo,
    nodoDivElementoHermanoIzquierdo,
    posicionDivElementoHermanoIzquierdo
  );
  //borro el elemento del padre que pasa al nodo en underflow
  borrarElementoDeNodoHojaSinUnder(
    alturaDivElementoPadre,
...
function redistribuirAIzqConHijos(
  altura,
  nodo,
  posicionElemento,
  elementoIntermedioPadre,
...
function redistribuirADerConHijos(
  altura,
  nodo,
  posicionElemento,
  elementoIntermedioPadre,
  ...
  centrarHijosEnFuncionAPadre(nodoElementos.id);
  centrarArbol(padreNodos);
  setTimeout("moverArbolParaScroll()", 10);
  if (posicionNodo == "extremo") {
    var mensaje = "- Redistribuci\u00f3n a derecha.";
  } else {
    var mensaje = "- Redistribuci\u00f3n a derecha.";
...
function borrarFusionConHermanoDerecho(
  altura,
  nodo,
  posicionElemento,
  elementoIntermedioPadre
...
function borrarFusionConHermanoIzquierdo(
  altura,
  nodo,
  posicionElemento,
  elementoIntermedioPadre
...
function fusionConHermanoDerecho(
  altura,
  nodo,
  posicionElemento,
  elementoIntermedioPadre
  ...
  if (nodoElementos.childNodes.length == 1) {
    //no tiene hijos
    if (padreNodos.childNodes.length == 3 && altura == 1) {
      //el padre tiene solo dos hijos que son los que estan fusionando y estan en la altura uno
      //paso los elementos de los nodos que fusionan al padre
      pasarElementosDeNodo(contenedorElementos, contenedorElementosPadre);
      pasarElementosDeNodo(
        contenedorElementosHermanoDerecho,
        contenedorElementosPadre
      );
      //borro los enlaces
      borrarEnlaces();
      ...
      $("#" + idNodoElementos).animate({ top: 0, left: 0 }, 1250, function () {
        padreNodos.removeChild(nodoElementos);
      });
      $("#" + idNodoHermanoDerecho).animate(
        { top: 0, left: 0 },
        1250,
        function () {
          padreNodos.removeChild(nodoHermanoDerecho);
          var container = document.getElementById("contenedor");
          posicionContenedor = position(container);
          rotularNodo(padreNodos, posicionContenedor);
        }
      );
      var mensaje =
        "- Fusi\u00f3n con hermano derecho. Disminuci\u00f3n de altura.";
      mostrarMensajeOperacion(mensaje);
      habilitarBotones();
    } //el padre tiene mas de dos hijos o es altura mayor a uno
    else {
      ...
      idContenedorElementosHermanoDerecho =
        contenedorElementosHermanoDerecho.id;
      var alturaDivDestino = idContenedorElementosHermanoDerecho[1];
      var nodoDivDestino = idContenedorElementosHermanoDerecho.substr(
        2,
        idContenedorElementosHermanoDerecho.length - 2
      );
    ...
    if (padreNodos.childNodes.length == 3 && altura == 1) {
      ...
      if (padreNodos.childNodes.length == 3 && altura != 1) {
        //el padre tiene solo dos hijos que son los que estan fusionando y estan en altura mayor a 1
...
function fusionConHermanoIzquierdo(
  altura,
  nodo,
  posicionElemento,
  elementoIntermedioPadre,
  ...
  if (nodoElementos.childNodes.length == 1) {
    //no tiene hijos
    if (padreNodos.childNodes.length == 3 && altura == 1) {
      //el padre tiene solo dos hijos que son los que estan fusionando
      //paso los elementos de los nodos que fusionan al padre
      pasarElementosDeNodo(contenedorElementos, contenedorElementosPadre);
      pasarElementosDeNodo(
        contenedorElementosHermanoIzquierdo,
      ...
      $("#" + idNodoHermanoIzquierdo).animate(
        { top: 0, left: 0 },
        1250,
        function () {
      ...
      if (posicionNodo == "extremo") {
        var mensaje =
          "- Fusi\u00f3n con hermano izquierdo. Disminuci\u00f3n de altura.";
      } else {
        var mensaje =
          "- Fusi\u00f3n con hermano izquierdo. Disminuci\u00f3n de altura.";
      }
      mostrarMensajeOperacion(mensaje);
      habilitarBotones();
    } //el padre tiene mas de dos hijos o la altura es mayor a uno
    else {
      ...
      idContenedorElementosHermanoIzquierdo =
        contenedorElementosHermanoIzquierdo.id;
      var alturaDivDestino = idContenedorElementosHermanoIzquierdo[1];
      var nodoDivDestino = idContenedorElementosHermanoIzquierdo.substr(
        2,
        idContenedorElementosHermanoIzquierdo.length - 2
      );
      ordenarNodo(alturaDivDestino, nodoDivDestino, elementoIntermedioPadre);
      borrarEnlaces();
      //guardo el numero de nodo reutilizable
...
function decrementarIdAHermanosPosteriores(numeroNodo, altura) {
  for (
    var j = 172;
    j != 0;
    j-- //172 es el numero maximo de nodos con orden 9 y altura 4
  ) {
    var idHermano = "N" + altura + j;
    var divHermano = document.getElementById(idHermano);
    if (divHermano != undefined) {
  ...
  for (
    ...
    if (divHermano != undefined) {
      ...
      for (var h = 0; h <= divElementos.childNodes.length - 1; h++) {
        ...
        if (idElemento[0] == "P") {
          divElemento.id =
            "P" + altura + parseInt(j - 1) + idElemento[idElemento.length - 1];
        } else {
          divElemento.id =
            "" + altura + parseInt(j - 1) + idElemento[idElemento.length - 1];
        }
...
function intercambiarElementos(
  alturaGraficoMCD,
  nodoGraficoMCD,
  posicionElementoGraficoMCD,
  alturaGrafico,
  nodoGrafico,
  posicionElementoGrafico
) {
  var idDivElementoMCD =
    "" + alturaGraficoMCD + nodoGraficoMCD + posicionElementoGraficoMCD;
  var divMCD = document.getElementById(idDivElementoMCD);
  var elementoMasChicoDerecho = divMCD.firstChild.nodeValue;
  var idElementoABorrar =
    "" + alturaGrafico + nodoGrafico + posicionElementoGrafico;
  ...
  $("#" + idElementoABorrar + "").effect(
    ...
    function () {
      ...
      setTimeout("ejecutarProximaAnimacion()", 10);
    }
  );
}
...
function finalizarPartirNodo(
  idDivEnglobaDesbordado,
  idDivNuevoNodoIzquierdo,
  idDivNuevoNodoDerecho,
  idDivPadre,
  numeroNodo,
  alturaNodo
) {
  ...
  /*borro los elementos de explicacion*/
  divExplicacionIzquierdo = document.getElementById(
    "explicacionNuevoNodoIzquierdo"
  );
  divParcialIzquierdo.removeChild(divExplicacionIzquierdo);
  divExplicacionDerecho = document.getElementById(
    "explicacionNuevoNodoDerecho"
  );
