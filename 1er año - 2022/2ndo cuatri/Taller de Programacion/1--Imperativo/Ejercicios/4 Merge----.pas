{ Una biblioteca nos ha encargado procesar la información de los préstamos realizados durante cada el año 2021. De cada préstamo se conoce el ISBN del libro, 
...
Procedure minimo (var v: vector; var prestamoMin: prestamoAGuardar);
   ...
   Begin
     ...
     begin
       if (v[i] <> nil) 
       then if (v [i]^.dato.ISBN <= prestamoMin.ISBN) 
            then begin
                   indiceMin:= i;
                   prestamoMin:= v [i]^.dato;  
                 end; 
     end;
     if (prestamoMin.ISBN <> valorGrande) 
